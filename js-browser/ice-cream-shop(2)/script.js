const cardsData = [
  ...
  const renderCards = function(product) {
    const listItem = makeElement('li', 'good');
    
    const title = makeElement('h2', 'good__description', product.text);
    ...
    listItem.classList.add(inStock);
    
    if (product.isHit) {
      listItem.classList.add('good--hit');
      const specialPrice = makeElement('p', 'good__special-offer', product.specialOffer);
  ...
  for (let i = 0; i < cardsData.length; i++) {
    const cardItem = renderCards(cardsData[i]);
